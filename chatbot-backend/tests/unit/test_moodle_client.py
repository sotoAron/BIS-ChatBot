...
@pytest.mark.asyncio
async def test_get_user_courses_success(moodle_client, httpx_mock):
    httpx_mock.add_response(
        url="http://moodle-test/webservice/rest/server.php",
        method="POST",
        json=[{"id": 1, "fullname": "Matemática"}]
    )
...
@pytest.mark.asyncio
async def test_download_file(moodle_client, httpx_mock):
    httpx_mock.add_response(
        url="http://moodle-test/file.pdf?token=read-token",
        method="GET",
